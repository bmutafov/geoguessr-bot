...
export async function updateChallengeResults({ challengeToken, message }: UpdateResultParams) {
	...
	try {
		result = await axios.get<ChallengeResultsResponse>(getResultUrl(challengeToken), {
	...
	message.edit(`New challenge: https://www.geoguessr.com/challenge/${challengeToken} \n${table}`);
